"""v2.75.0 T6-T8 — split commands/vg/debug.md into _shared/debug/*.md sub-files.

Same pattern as scripts/v275_split_specs.py.
"""
...
GROUPS = {
    "preflight": {
        ...
        "shared_rel": "_shared/debug/preflight.md",
        "steps": ["0_parse_and_classify"],
        "blurb": "Includes 1 step: 0_parse_and_classify.",
    },
    "discovery-and-fix": {
        "title": "### Discovery + hypothesize + fix (extracted v2.75.0 T7)",
        "shared_rel": "_shared/debug/discovery-and-fix.md",
        "steps": ["1_discovery", "2_hypothesize_and_fix"],
        "blurb": "Includes 2 steps: 1_discovery, 2_hypothesize_and_fix.",
    },
    "verify-and-close": {
        "title": "### Verify + close (extracted v2.75.0 T8 — final)",
        "shared_rel": "_shared/debug/verify-and-close.md",
        "steps": ["3_verify_and_loop", "4_complete"],
        "blurb": "Includes 2 steps: 3_verify_and_loop, 4_complete.",
    },
}


CANONICAL_FILE = Path("commands/vg/debug.md")
MIRROR_FILE = Path(".claude/commands/vg/debug.md")
...
def split_group(group_key: str) -> tuple[int, int, int]:
    ...
    sub_header = (
        f"<!-- v2.75.0 T6-T8 extraction — verbatim step blocks from "
        f"commands/vg/debug.md -->\r\n"
...
def main(argv: list[str]) -> int:
    if len(argv) != 2:
        print("usage: python3 scripts/v275_split_debug.py <group>", file=sys.stderr)
        print(f"valid groups: {list(GROUPS)}", file=sys.stderr)
        return 2
    group_key = argv[1]
    before, after, sub = split_group(group_key)
    print(
        f"{group_key}: debug.md {before} -> {after} lines | "
