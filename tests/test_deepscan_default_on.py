...
def test_changelog_documents_breaking_change():
    ...
    # v2.65.0 entry must mention BREAKING + deepscan
    v2_65 = re.search(r"v2\.65\.0.*?(?=v2\.6[4-9]|v2\.[7-9]|##|\Z)", body, re.DOTALL)
    if v2_65:
        section = v2_65.group(0)
        assert ("BREAKING" in section.upper() or "breaking" in section), \
            "v2.65.0 CHANGELOG must mention BREAKING change"
        assert "deepscan" in section.lower(), \
