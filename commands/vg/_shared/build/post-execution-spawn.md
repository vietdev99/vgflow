# build post-execution — pre-spawn checklist + Spawn site

Sub-ref of `post-execution-overview.md`. This file contains the
orchestrator-side bash gates that run BEFORE spawning the
`vg-build-post-executor` subagent, then the single Agent() spawn
call and Codex runtime variant.

Read `post-execution-overview.md` first for the HARD-GATE + Step
ordering. Read `post-execution-delegation.md` for the input/output JSON
contract of the subagent itself. Read `post-execution-validation.md`
for what to do AFTER the spawn returns.

**HARD-GATE preserved (R6 Task 3 single-spawn enforcement):** the
spawn-guard hard-denies a 2nd `Agent(subagent_type="vg-build-post-executor")`
call. Counter persisted at `.vg/runs/<run_id>/.post-executor-spawns.json`.
You MUST narrate via `bash scripts/vg-narrate-spawn.sh` (R1a UX baseline
Req 2) — `spawning` before, `returned` on success, `failed` on error.
