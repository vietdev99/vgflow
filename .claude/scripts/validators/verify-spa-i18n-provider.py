...
# B89 v4.66.0 fix: original regex `createRoot\s*\([^)]*\)\s*\.\s*render\s*\(`
# failed on nested parens. `createRoot(document.getElementById('root')).render(`
# has `()` inside the argument — `[^)]*` stops at the FIRST `)` (inside
# getElementById), and the rest of the pattern then sees `).render(` which
# does NOT match `\)\s*\.\s*render`. Falls through to "no createRoot call"
# → PASS bypass. Real PrintwayV3 main.tsx uses a pre-declared `rootElement`
# variable (no nested parens) so the upstream PR's smoke pass was misleading.
# Replacement: detect both tokens independently. Order check skipped — no
# valid SPA puts `.render(` before `createRoot(`.
CREATE_ROOT_TOKEN_RE = re.compile(r"createRoot\s*\(")
RENDER_TOKEN_RE = re.compile(r"\.\s*render\s*\(")

# Legacy alias kept so any consumer importing CREATE_ROOT_RE doesn't crash;
# its behavior is irrelevant — only the helper below is used internally.
CREATE_ROOT_RE = CREATE_ROOT_TOKEN_RE


def _has_create_root_render(text: str) -> bool:
    return bool(CREATE_ROOT_TOKEN_RE.search(text)) and bool(RENDER_TOKEN_RE.search(text))
...
def check_app_entry(main_file: Path):
    ...
    if not _has_create_root_render(text):
