...
Render promoted bootstrap rules tagged for the `review` step BEFORE
STEP 3 (browser discovery) spawns `vg-review-browser-discoverer`, before
STEP 4 (lens dispatch) spawns Haiku scanners, and before STEP 5 (findings
fix-loop) spawns the scope-fix subagent. Closes R9-B gap from codex audit
(2026-05-05): review subagents previously spawned WITHOUT seeing learned
rules — repeated past mistakes the harness already learned in earlier
phases.

```bash
# Source helper + render block tagged for `review` target_step.
# Helper internally filters scope DSL → only review-relevant rules render.
...
Downstream spawn sites (`review.md` STEP 3 vg-review-browser-discoverer,
STEP 4 lens scanners via `spawn_recursive_probe.py`, STEP 5 fix-loop)
embed the block in their Agent prompts:
...
Empty match → helper emits `(no project-specific rules match this step
...)` placeholder; the static integrity checker
(`vg_bootstrap_verify_injection`) then sees the tag and treats the spawn
as injected (anti-silent-skip).
