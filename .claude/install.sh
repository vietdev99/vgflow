...
# Agents live at .claude/agents/{name}.md OR .claude/agents/{name}/SKILL.md
# (directory form). Skill frontmatter `agent: vg-*` spawns these with the
# green tag. Thin-shells fail-loud if the calling skill forgets to inject
# the corresponding rule block.
#
# B100 v4.69.3 (issue #203): pre-B100 only copied top-level *.md, missing
# all directory-form subagents (vg-test-goal-verifier/, vg-test-fixer/,
# vg-test-codegen/, vg-blueprint-contracts/, vg-blueprint-fe-contracts/,
# vg-blueprint-planner/, vg-blueprint-workflows/, vg-build-post-executor/,
# vg-build-task-executor/, vg-accept-cleanup/, vg-accept-uat-builder/,
# vg-field-test-analyzer/). Result: skills referencing those agents either
# blocked or fell through to general-purpose (96.5% inline-skip rate per
# Codex review).
if [ -d "$SCRIPT_DIR/agents" ]; then
  mkdir -p "$TARGET/.claude/agents"
  # Top-level .md (vg-planner.md, vg-plan-checker.md)
  cp "$SCRIPT_DIR/agents/"*.md "$TARGET/.claude/agents/" 2>/dev/null || true
  AGENT_MD_COUNT=$(ls "$TARGET/.claude/agents/"*.md 2>/dev/null | wc -l | tr -d ' ')
  # B100: subagent dirs (each containing SKILL.md)
  AGENT_DIR_COUNT=0
  while IFS= read -r agent_dir; do
    [ -f "$agent_dir/SKILL.md" ] || continue
    agent_name="$(basename "$agent_dir")"
    mkdir -p "$TARGET/.claude/agents/$agent_name"
    cp -R "$agent_dir"/. "$TARGET/.claude/agents/$agent_name/"
    AGENT_DIR_COUNT=$((AGENT_DIR_COUNT + 1))
  done < <(find "$SCRIPT_DIR/agents" -mindepth 1 -maxdepth 1 -type d | sort)
  AGENT_TOTAL=$((AGENT_MD_COUNT + AGENT_DIR_COUNT))
  echo "  → ${AGENT_TOTAL} VG agent(s) installed (${AGENT_MD_COUNT} top-level + ${AGENT_DIR_COUNT} subagent dirs — B100 v4.69.3)"
fi
